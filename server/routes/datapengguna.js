...
router.put("/:id_pengguna", async (req, res) => {
  ...
  let query =
    "UPDATE pengguna SET username = ?, role = ? WHERE id_pengguna = ?";
  let params = [username, role, id_pengguna];

  if (password) {
    const hashedPassword = await bcrypt.hash(password, 10);
    query =
      "UPDATE pengguna SET username = ?, password = ?, role = ? WHERE id_pengguna = ?";
...
router.post("/migratePendaftaran", (req, res) => {
  const query = `
  ...
  db.query(query, (err, results) => {
    ...
    res
      .status(201)
      .json({
        message: "Data pendaftaran berhasil dimigrasikan ke pengguna.",
        results,
      });
  });
});
